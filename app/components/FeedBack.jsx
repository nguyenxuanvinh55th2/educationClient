...
class FeedBack extends React.Component {
    ...
    componentDidUpdate() {
      if (this.gridOptions.api) {
        this.gridOptions.api.setFloatingBottomRowData(this.renderFooterData(this.props.accountingObjects));
      }
    }
      ...
      if (remove) {
        this.props.removeTeamBuilding(userId, _id).then(() => {
          ...
          Meteor.call('sendNotification', {
            note: Meteor.user().username + ' ' + 'xóa phản hồi của khách hàng ' + name,
            isManage: true
          }, (err, res) => {
            if (err) {
              alert(err);
    ...
    verifyRow(userId, _id, name) {
      this.props.verifyTeamBuilding(userId, _id).then(() => {
        ...
        Meteor.call('sendNotification', {
          note: Meteor.user().username + ' ' + 'duyệt phản hồi của khách hàng ' + name,
          isManage: true
        }, (err, res) => {
          if (err) {
            alert(err);
    ...
    render() {
        let { accountingObjects } = this.props;
        if (Meteor.userId()) {
            if (!accountingObjects) {
                ...
                let columnDefs= [
                    ...
                    {
                        headerName: 'Đánh giá/góp ý', field: "feedBack",
                        filterParams: {filterOptions: ['contains', 'notContains', 'startsWith', 'endsWith']}, width: this.state.width, editable: false, filter: 'text'
                    },
                    {
                          ...
                          <div style={{height: 36, display: 'flex', flexDirection: 'row', backgroundColor: '#f5f5f5', border: '1px solid #d1d1d1', borderBottom: 'none', padding: 5, position: 'relative', justifyContent: 'space-between'}}>
                              <div style={{
                                ...
                                <ol className="breadcrumb" style={{
                                  ...
                                  <li>
                                    <a onClick={() => browserHistory.push('/feedBack')}>Phản hồi</a>
                                  </li>
                                </ol>
                              </div>
                              <div>
...
const FeedBackWithData = compose (
    graphql(VERIFY, {
        props:({mutate})=>({
...
export default createContainer((ownProps) => {
  ...
  return {
    accountingObjects: AccountingObjects.find({isFeedBack: true}, {sort: {createdAt: -1}}).fetch()
  }
}, FeedBackWithData);
