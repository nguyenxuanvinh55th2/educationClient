...
class Advertisements extends React.Component {
  constructor(props) {
    super(props);
    this.state = {open: false, type: '', height: window.innerHeight, openReview: false};
  }
          ...
          <div>
            <div style={{display: 'flex', flexDirection: 'row', justifyContent: 'space-between'}}>
              <ol className="breadcrumb" style={{marginBottom: 0, backgroundColor: 'white'}}>
                ...
                <li>
                  <a onClick={() => browserHistory.push('/imageManager')}>Hình ảnh</a>
                </li>
              </ol>
                      ...
                      <CardMedia style={{height: 300, display: 'flex', flexDirection: 'column', justifyContent: 'center', backgroundColor: 'black'}}
                        overlay = {
                          <CardTitle title={'Tên: ' + item.name} subtitle={(
                              <div>
                                { 'Loại: ' + (
                                  item.type === 'countryTour' ? 'Tour trong nước' :
                                  item.type === 'foreignTour' ? 'Tour nước ngoài' :
                                  item.type === 'teamBuilding' ? 'Team building' :
                                  item.type === 'event' ? 'Sự kiện' :
                                  item.type === 'promotion' ? 'Khuyến mãi' : 'Liên hệ'
                                )}
                                <br/>
                                {'Tiêu đề: ' + item.title}
                                <br/>
                        ...
                        <button className="btn btn-danger" onClick={() => {
                          ...
                          if (remove) {
                            this.props.removeAdvertise(Meteor.userId(), item._id).then(() => {
                              ...
                              this.props.addNotificationMute({fetchData: true, message: 'Xóa ảnh thành công', level: 'success'});
                              Meteor.call('sendNotification', {
                                note: Meteor.user().username + ' ' + 'xóa ảnh ' + item.name,
                                isManage: true
                              }, (err, res) => {
                                if (err) {
                                  alert(err);
