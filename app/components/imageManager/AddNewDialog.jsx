...
class AddNew extends React.Component {
  constructor(props) {
    ...
    if(props.type === 'update') {
      this.state = props.updateItem;
        ...
        this.state = {image: null, name: '', type: 'countryTour', _id: '', isShow: false, title: ''}
    }
  }
  ...
  insertNew() {
    ...
    this.props.insertOrUpdateAdvertise(Meteor.userId(), _id , info).then(() => {
      ...
      this.props.refetch();
      this.props.addNotificationMute({fetchData: true, message: 'Lưu ảnh thành công', level: 'success'});
      if(!_id) {
        Meteor.call('sendNotification', {
          note: Meteor.user().username + ' ' + 'lưu ảnh ' + this.state.name,
          isManage: true
        }, (err, res) => {
          if (err) {
            alert(err);
          ...
          Meteor.call('sendNotification', {
            note: Meteor.user().username + ' ' + 'sửa ảnh ' + this.state.name,
            isManage: true
          }, (err, res) => {
            if (err) {
              alert(err);
  ...
  render() {
    let { name, type, isShow, title } = this.state;
    let { height } = this.props;
    return (
      <div>
          <div className="modal-dialog" style={{width: 'auto', margin: 0}}>
              <div className="modal-header">
                  <h4 className="modal-title">Thêm ảnh</h4>
              </div>
                    ...
                    <div style={{display: 'flex', flexDirection: 'row', justifyContent: 'space-between'}}>
                      <div style={{width: '100%'}}>
                        <div className={name ? "form-group" : "form-group has-error"} style={{width: '100%', margin: 0, display: 'flex', flexDirection: 'row', justifyContent: 'flex-start'}}>
                          ...
                          <div style={{width: '80%'}}>
                            <input type="text" className="form-control" value={name} style={{width: '100%'}} placeholder={'Nhập tên'} onChange={({target}) => {
                        ...
                        <div className={title ? "form-group" : "form-group has-error"} style={{width: '100%', margin: 0, display: 'flex', flexDirection: 'row', justifyContent: 'flex-start'}}>
                          <label style={{width: '20%', paddingLeft: 0, textAlign: 'right', paddingRight: 10}}>
                            Tiêu đề
                          </label>
                          <div style={{width: '80%'}}>
                            <input type="text" className="form-control" value={title} style={{width: '100%'}} placeholder={'Nhập tiêu đề'} onChange={({target}) => {
                        ...
                        <div className={type ? "form-group" : "form-group has-error"} style={{width: '100%', margin: '0px 0px 15px 0px', display: 'flex', flexDirection: 'row', justifyContent: 'flex-start'}}>
                          ...
                          <select value={this.state.type} className="form-control custom" style={{width: '80%'}} onChange={({target}) => {
                            ...
                            <option value="countryTour">Tour trong nước</option>
                            <option value="foreignTour">Tour nước ngoài</option>
